...
- [ ] T001 [US1] Update `submitForm` hook execution to map payload payload identically to the immediate values instead of exclusively off draftRef snapshot in `src/presentation/view-models/use-submission.ts`
- [ ] T002 [US1] Secure validation rejection blocks by validating explicit length dependencies on collection extraction in `src/domain/use-cases/client/submit-form.ts`
- [ ] T003 [US1] Protect state hydration sequence ordering during background submission refreshes in `src/presentation/view-models/use-submission.ts`
